  ...
  return (
    <Layout noPadding>
      <div className="flex-1 flex flex-col h-full bg-white relative overflow-hidden">
        {/* Mapa de Fundo - Menor no Mobile para dar espaço ao formulário */}
        <div className="h-[40%] w-full z-0 relative border-b border-gray-100">
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            {step === 1 ? (
              <div className="flex-1 flex flex-col">
                <div className="flex-1 overflow-y-auto px-8 pt-4 pb-4 space-y-6 no-scrollbar">
                  <div>
                  ...
                  <div className="space-y-4">
                    <div className="bg-gray-50 p-5 rounded-3xl border border-gray-100 focus-within:border-black transition-colors">
                      ...
                      <input 
                        className="w-full bg-transparent outline-none font-black text-gray-900 text-lg placeholder:text-gray-200" 
                        placeholder="Ex: Caixa, Garçom..." 
                        value={title} 
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="bg-gray-50 p-5 rounded-3xl border border-gray-100">
                        ...
                        <div className="flex items-center justify-between">
                           <button onClick={() => setDuration(Math.max(1, duration - 1))} className="w-6 h-6 rounded-lg bg-white border flex items-center justify-center font-black shadow-sm text-gray-400">-</button>
                           <span className="font-black text-sm">{duration}h</span>
                           <button onClick={() => setDuration(duration + 1)} className="w-6 h-6 rounded-lg bg-white border flex items-center justify-center font-black shadow-sm text-gray-400">+</button>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="p-6 bg-white border-t border-gray-50 shrink-0">
                  <button 
                    onClick={() => setStep(2)} 
                    disabled={!title} 
                    className="w-full bg-black text-white h-16 rounded-[1.2rem] font-black text-sm uppercase tracking-widest disabled:opacity-20 active:scale-95 transition-all shadow-2xl"
                  >
                    Próximo
                  </button>
                </div>
              ...
              <div className="flex-1 flex flex-col overflow-hidden">
                <div className="px-8 pt-4 pb-2 shrink-0">
                  <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Opções Disponíveis</h3>
                </div>
                
                {/* Lista de Categorias Rolável */}
                <div className="flex-1 px-4 space-y-3 overflow-y-auto no-scrollbar py-2">
                  {tiers.map(tier => (
                    <button 
                      key={tier.id} 
                      onClick={() => setSelectedTier(tier.id)} 
                      className={`w-full flex items-center justify-between px-6 py-5 rounded-[1.5rem] border-2 transition-all duration-300 ${selectedTier === tier.id ? 'border-black bg-gray-50 shadow-lg scale-[1.01]' : 'border-transparent bg-white shadow-sm opacity-60'}`}
                    >
                      <div className="flex items-center gap-4 text-left">
                        <div className="w-12 h-12 bg-white border border-gray-100 rounded-xl flex items-center justify-center text-2xl shadow-sm shrink-0">{tier.icon}</div>
                        <div>
                      ...
                      <div className="text-right">
                        <p className="font-black text-lg text-gray-900 leading-none">R$ {tier.price.toFixed(0)}</p>
                      </div>
                    </button>
                  ))}
                  <div className="h-4"></div> {/* Espaçador final da lista */}
                </div>

                {/* Botão de Confirmação Fixo na Base */}
                <div className="p-6 bg-white border-t border-gray-100 shrink-0 shadow-[0_-10px_30px_rgba(0,0,0,0.03)]">
                  <button onClick={handleConfirm} className="w-full bg-black text-white h-16 rounded-[1.2rem] font-black text-base uppercase tracking-widest active:scale-95 transition-all shadow-2xl">
                    Confirmar MeUp
                  </button>
                </div>
              </div>
