...
const NewJob: React.FC = () => {
  ...
  const [step, setStep] = useState(1); // 1: Detalhes, 2: Seleção Uber Style
  ...
  const tiers = [
    { 
      id: 'Standard', 
      name: 'MeUp Standard', 
      desc: 'Profissionais avaliados • 4 min', 
      price: 25 * duration, 
      icon: '👤',
      tag: 'MAIS POPULAR'
    },
    { 
      id: 'Expert', 
      name: 'MeUp Especialista', 
      desc: 'Experiência comprovada • 6 min', 
      price: 35 * duration, 
      icon: '⭐',
      tag: null
    },
    { 
      id: 'Master', 
      name: 'MeUp Master', 
      desc: 'Líderes e Gestores • 10 min', 
      price: 55 * duration, 
      icon: '🏆',
      tag: null
    }
  ...
  return (
    <Layout noPadding>
      <div className="flex-1 flex flex-col h-full bg-white relative overflow-hidden">
        ...
        {/* MAPA (Topo - Ajustado para dar mais ar ao bottom sheet) */}
            ...
            {step === 1 ? (
              <div className="flex-1 overflow-y-auto px-8 pt-4 pb-8 space-y-5 animate-in fade-in slide-in-from-bottom duration-300 no-scrollbar">
                ...
                <div className="space-y-3">
                  <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                    ...
                    <input 
                      ...
                      placeholder="Ex: Caixa para Fechamento"
                      value={title}
                  ...
                  <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                    ...
                    <select 
                      className="w-full bg-transparent outline-none font-bold text-gray-900 appearance-none"
                      value={loc}
                ...
                <button 
                  onClick={() => setStep(2)}
                  disabled={!title}
                  className="w-full bg-black text-white py-4.5 rounded-2xl font-black text-sm uppercase tracking-widest disabled:opacity-30 active:scale-95 transition-all shadow-xl mt-2"
                >
                  Ver Categorias
                </button>
              </div>
            ) : (
              /* SELEÇÃO UBER STYLE - OTIMIZADA PARA SEM SCROLL E SEM PAGAMENTO */
              <div className="flex-1 flex flex-col animate-in fade-in slide-in-from-right duration-300">
                ...
                {/* Lista de Categorias - Justificada para preencher o espaço sem scroll */}
                <div className="flex-1 px-4 flex flex-col justify-around py-2">
                  {tiers.map(tier => (
                    <button
                      key={tier.id}
                      onClick={() => setSelectedTier(tier.id)}
                      className={`w-full flex items-center justify-between px-5 py-3 rounded-2xl border-2 transition-all duration-200 ${
                        selectedTier === tier.id ? 'border-black bg-gray-50 shadow-md ring-1 ring-black/5' : 'border-transparent hover:bg-gray-50/50'
                      }`}
                    >
                      <div className="flex items-center gap-4 text-left">
                        <div className="w-12 h-12 bg-white border border-gray-100 rounded-2xl flex items-center justify-center text-2xl shadow-sm shrink-0">
                          {tier.icon}
                        </div>
                        <div>
                          ...
                          {tier.tag && (
                            <span className="text-[8px] font-black text-blue-600 uppercase tracking-tighter bg-blue-50 px-1.5 py-0.5 rounded mt-1.5 inline-block border border-blue-100">
                              {tier.tag}
                            </span>
                          )}
                        </div>
                      ...
                      <div className="text-right shrink-0">
                        <p className="font-black text-base text-gray-900 leading-none">R$ {tier.price.toFixed(2)}</p>
                      </div>
                    </button>
                  ))}
                </div>
                ...
                <div className="px-6 pt-2 pb-8 bg-white shrink-0">
                  ...
                  >
                    Confirmar {selectedTier.toUpperCase()}
                  </button>
                </div>
              </div>
            )}
